...
import os
import time
# URL Encoding ke liye (WhatsApp message banane ke liye)
import urllib.parse 
...
# WhatsApp data store karne ke liye
...
def create_pdf(mill_id, date, time_str, weight, rate, price, munim_name):
    ...
    c.setLineWidth(2)
    c.rect(50, height - 350, width - 100, 300)
    ...
    c.setLineWidth(1)
    c.line(60, height - 135, width - 60, height - 135)
    y = height - 170
    x_left = 100
    x_right = 350
    c.setFont("Helvetica", 14)
    ...
    c.drawCentredString(width / 2, height - 330, "Generated by Nia Rice Mill SaaS")
    c.save()
    buffer.seek(0)
    return buffer
...
def save_data_secure(wazan, rate):
    ...
    try:
        ...
        # WhatsApp Data save kar rahe hain
        st.session_state.last_data = {
            "w": wazan, "r": rate, "p": price, "d": d_date, "t": d_time, "m": my_mill_id
        }
        return pdf_file
    except: return None
...
if not st.session_state.logged_in:
    ...
    tab1, tab2 = st.tabs(["🔑 Login", "✨ Activate"])
    with tab1:
        with st.form("login"):
...
with c1:
    ...
    if st.session_state.step == 1:
        audio = mic_recorder(start_prompt="🔴 Speak", stop_prompt="⏹️ Stop", key='rec1')
        if audio:
            try:
                webm = audio['bytes']
...
Generated via Nia SaaS 🚀"""
            
            # Encode message for URL (Special characters handle karne ke liye)
            encoded_msg = urllib.parse.quote(msg)
            whatsapp_url = f"https://wa.me/?text={encoded_msg}"
            
            # Show Green WhatsApp Button
            st.link_button("📲 Send on WhatsApp", whatsapp_url, use_container_width=True)
